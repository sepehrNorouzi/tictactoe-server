...
exports.match_make_cancel = (socket, io, data) => {
	const socket_index = match_wait.indexOf(socket);
	if (socket_index == -1) {
		return socket.emit("match_wait_cancel", { message: "Canceled." });
	}
	match_wait.splice(socket_index, 1);
	return socket.emit("match_wait_cancel", { message: "Canceled." });
};
